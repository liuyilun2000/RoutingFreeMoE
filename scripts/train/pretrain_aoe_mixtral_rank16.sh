...
: "${WANDB_API_KEY:?set WANDB_API_KEY in your shell or .env before submitting}"
...
RUN_NAME="S_aoe_${config}_rank16_lr_${LEARNING_RATE}"

MODEL_DIR=${1:-./config/AoE_${config}_rank16}
...
torchrun --nproc_per_node 4 scripts/train/pretrain_aoe_mixtral.py \
  --model-dir "$MODEL_DIR" \
