...
def train(
    ...
    if config.num_experts_per_tok != num_experts_per_tok:
        raise ValueError(f"Number of experts per token in config ({config.num_experts_per_tok}) does not match the provided value ({num_experts_per_tok})")
    ...
    # Note: Don't manually wrap with DDP - Trainer handles this automatically
    # For MoE models, Trainer will use find_unused_parameters=True internally
    ...
    training_args = transformers.TrainingArguments(
        ...
        ddp_backend="nccl",
        ddp_find_unused_parameters=True,  # Required for MoE models where not all experts are used
    ...
    # Note: This should be done before training, not after trainer creation
    ...
    # After training, trainer.model is the best model (if load_best_model_at_end=True)
    ...
    # Save the best model (trainer.model is already the best model after training)
    if local_rank <= 0:
...
def main():
    parser = argparse.ArgumentParser(description="Train Mixtral baseline model")
    ...
    parser.add_argument("--lr", type=float, default=1e-3,
                      help="Learning rate")
    parser.add_argument("--wandb-project", type=str, default="mixtral-baseline",
