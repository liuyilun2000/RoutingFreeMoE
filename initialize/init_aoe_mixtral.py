...
from routing_free.mixtral_aoe import AoEMixtralForCausalLM, AoEMixtralConfig
...
def initialize_model(
    ...
    # Load config
    config = AoEMixtralConfig.from_pretrained(config_json)
    ...
    config.n_experts = num_local_experts

    print(f"Initializing AoE Mixtral with config: {config}")
    model = AoEMixtralForCausalLM(config=config)
...
def main():
    parser = argparse.ArgumentParser(description="Initialize an AoE Mixtral model")
    ...
    parser.add_argument("--num-experts-per-tok", type=int, required=True, help="Number of experts per token (top-K)")
    parser.add_argument("--intermediate-size", type=int, required=True, help="Intermediate size")
    parser.add_argument("--gate-proj-rank", type=int, default=32, help="Gate projection rank for low-rank self-evaluation")
