...
from routing_free.mixtral_remoe import ReMoEMixtralForCausalLM, ReMoEMixtralConfig
...
def initialize_model(
    ...
    # Load config
    config = ReMoEMixtralConfig.from_pretrained(config_json)
    config.vocab_size = len(tokenizer)
    config.num_hidden_layers = num_hidden_layers
    config.num_local_experts = num_local_experts
    config.num_experts_per_tok = num_local_experts  # ReMoE: ReLU handles sparsity
    config.intermediate_size = intermediate_size
    config.l1_coef = l1_coef
    config.density_target = density_target
    config.eta_coef = eta_coef

    print(f"Initializing ReMoE Mixtral with config: {config}")
    model = ReMoEMixtralForCausalLM(config=config)
...
def main():
    parser = argparse.ArgumentParser(description="Initialize a ReMoE Mixtral model")
    ...
    parser.add_argument("--l1-coef", type=float, default=0.01, help="L1 regularization coefficient for ReMoE router")
    parser.add_argument("--density-target", type=float, default=0.25, help="Target density (fraction of active experts)")
    parser.add_argument("--eta-coef", type=float, default=0.02, help="Adaptive lambda update rate")
